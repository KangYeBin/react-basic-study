...
const Login = () => {
  ...
  */
  ...
  // emailState에서 isValid 프로퍼티를 디스트럭처링
  ...
  useEffect(() => {
    ...
    //의존성 emailIsValid배열의 상태변수가 바뀔 때마다 useEffect가 재실행된다
  }, [emailIsValid, pwIsValid]);

  const emailChangeHandler = (e) => {
    // reducer의 상태 변경은 dispatch 함수를 통해서 처리된다
    // dispatch 함수의 매개값 객체의 key는 정해진 것이 아닌, reducer 함수에서 구분하기 위해 붙여주는 이름
    // 프로퍼티의 key와 value는 자유롭게 (정해져있지 않음)
    dispatchEmail({
      type: 'USER_INPUT',
      val: e.target.value,
    });
